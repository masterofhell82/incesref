...
const AppSidebar: React.FC = () => {
          ...
          {nav.subItems ? (
            ...
            >
              ...
              {(isExpanded || isHovered || isMobileOpen) && (
                <span className={`menu-item-text text-[16px]`}>{nav.name}</span>
              )}
              {(isExpanded || isHovered || isMobileOpen) && (
                <ChevronDownIcon
              ...
              <ul className="mt-2 ml-9 space-y-1">
                {nav.subItems.map((subItem) => (
                  <li key={subItem.name}>
                    <Link
                      href={subItem.path}
                      className={`menu-dropdown-item text-[16px] ${
  ...
  return (
    ...
    >
      <div
        style={styleSidebar}
        className={`flex py-6 ${!isExpanded && !isHovered ? 'md:justify-start' : 'justify-center'}`}
      >
        <Link href="/">
          {isExpanded || isHovered || isMobileOpen ? (
            <>
            ...
            <Image src="/images/logo/minilogo.svg" alt="Logo" width={32} height={32} />
          )}
        </Link>
      </div>
