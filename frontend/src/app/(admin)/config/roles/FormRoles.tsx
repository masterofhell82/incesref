...
const FormRoles = ({
  ...
  const getInitialForm = (data: Roles | null) => ({
    id: data?.id ?? null,
    nombre: data?.nombre ?? '',
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        if (dataForm.id) {
          await put(`${roles}/${dataForm.id}`, JSON.stringify(values));
          notify('success', 'Éxito', 'El rol ha sido actualizado exitosamente');
        } else {
          await post(roles, JSON.stringify(values));
          notify('success', 'Éxito', 'El rol ha sido registrado exitosamente');
        }
        action();
      } catch (error) {
  ...
  return <>
      <Modals
        title={isEdit ? `Editar Rol: ${data?.nombre}` : 'Agregar Rol'}
        isModalOpen={isOpen}
        handleCancel={action}
        width={500}
        footer={null}
