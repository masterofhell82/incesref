...
import { TrainingPrograms } from '@/interface/FormationsInterfaces';
...
const FormPrograms = ({
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        if (dataForm.id) {
          await put(`${programas}/${dataForm.id}`, JSON.stringify(values));
          notify('success', 'Éxito', 'El programa de formación ha sido actualizado exitosamente');
        } else {
          await post(programas, JSON.stringify(values));
          notify('success', 'Éxito', 'El programa de formación ha sido registrado exitosamente');
        }
        action();
      } catch (error) {
